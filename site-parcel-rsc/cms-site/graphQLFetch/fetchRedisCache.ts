...
export function createFetchRedisCache(fetch: Fetch, cache: { redis: Redis, fallbackCache: LRUCache<string, any>, ttl: number, revalidate: number }): Fetch {
...
async function serializeResponse(response: Response) {
    ...
    if (response.headers.get("content-type")?.includes("application/json")) {
        ...
        if (json?.errors) { // Must not cache GraphQL errors
            return false;
        }
    }
