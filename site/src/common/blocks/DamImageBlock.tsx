...
import { BreakpointValue } from "@src/theme";
...
export function createSizes(
    breakpointWidths: Partial<Record<"sx" | "sm" | "md" | "lg", string | number>>,
    defaultWidth: string | number = "100vw",
): string {
    const sizes: string[] = [];

    ["sx", "sm", "md", "lg"].forEach((breakpoint) => {
...
export function hasValidImage(data: DamImageBlockData): boolean {
    if (data.block) {
